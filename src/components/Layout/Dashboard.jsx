...
import {
  CheckSquare, Users, Calendar, ArrowRight,
...
export default function Dashboard() {
  ...
  async function carregarDados() {
    ...
    if (isGestor) {
      promises.push(supabase.from('despesas').select('*'))
    }
    const results = await Promise.all(promises)
    setData({
      clientes: results[0].data || [],
      tarefas: results[1].data || [],
      atendimentos: results[2].data || [],
      eventos: results[3].data || [],
      despesas: isGestor ? (results[4]?.data || []) : [],
    })
    setLoading(false)
  }
  ...
  const { clientes, tarefas, eventos, despesas } = data
  ...
  const tarefasAtrasadas = tarefasMinhas.filter(t => t.status === 'atrasada')
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Alertas */}
      {(tarefasAtrasadas.length > 0 || (isGestor && inadimplentes.length > 0)) && (
        <div className="space-y-2 mb-6">
          {tarefasAtrasadas.length > 0 && (
            <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
              <AlertTriangle className="w-4 h-4 flex-shrink-0" />
              <span>{tarefasAtrasadas.length} tarefa{tarefasAtrasadas.length > 1 ? 's' : ''} em atraso</span>
              <button onClick={() => navigate('/tarefas')} className="ml-auto text-red-600 font-medium flex items-center gap-1">
          ...
          {isGestor && inadimplentes.length > 0 && (
            <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 text-sm text-yellow-800">
              ...
              <button onClick={() => navigate('/honorarios')} className="ml-auto text-yellow-700 font-medium flex items-center gap-1">
                Ver <ArrowRight className="w-3 h-3" />
              </button>
            </div>
          )}
      ...
      {/* KPIs — diferentes para gestor e colaborador */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {isGestor ? <>
          ...
          <div className="stat-card cursor-pointer hover:shadow-md" onClick={() => navigate('/clientes')}>
            <div className="stat-label">Total de clientes</div>
            <div className="stat-value text-brand-700">{clientes.length}</div>
            <div className="stat-sub">clientes ativos</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Minhas tarefas</div>
            <div className="stat-value text-brand-700">{tarefasMinhas.filter(t => t.status !== 'concluida').length}</div>
            <div className="stat-sub">em aberto</div>
          </div>
        </>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <div className="card-header flex items-center justify-between">
            ...
            <button onClick={() => navigate('/tarefas')} className="text-xs text-brand-600 flex items-center gap-1">
            ...
            {tarefasMinhas.filter(t => t.status !== 'concluida').slice(0, 5).map(t => (
              <div key={t.id} className="px-5 py-3 flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className={stBadge[t.status] || 'badge-gray'}>{stLabel[t.status] || t.status}</span>
                </div>
              </div>
            ))}
        ...
        <div className="card">
          <div className="card-header flex items-center justify-between">
            ...
            <button onClick={() => navigate('/agenda')} className="text-xs text-brand-600 flex items-center gap-1">
