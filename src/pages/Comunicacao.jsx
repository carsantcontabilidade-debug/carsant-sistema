...
export default function Comunicacao() {
  ...
  async function gerarMensagemComIA() {
    ...
    setGerandoIA(true);
    try {
      // Normaliza cliente para o prompt
      const clienteNormalizado = {
        ...clienteForm,
        email: getEmail(clienteForm),
        regime_tributario: clienteForm.regime,
        honorario_mensal: clienteForm["valor_honorário"],
      };
      const prompt = TEMPLATES[form.template]?.promptIA(clienteNormalizado, form.canal);
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {form.template && clienteForm && (form.canal === "whatsapp" || form.canal === "email") && (
                        ...
                        >
                          {gerandoIA ? <><span className="animate-spin">⚙️</span> Gerando...</> : <>✨ Gerar com IA</>}
                        </button>
                      )}
                    </div>
                    <textarea
