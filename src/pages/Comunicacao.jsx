...
export default function Comunicacao() {
  ...
  const [modalTipo, setModalTipo] = useState("nova"); // nova | visualizar
  const [comunicacaoAtual, setComunicacaoAtual] = useState(null);
  const [loadingClientes, setLoadingClientes] = useState(true);
  const [loadingComms, setLoadingComms] = useState(false);
  const [gerandoIA, setGerandoIA] = useState(false);
  const [aba, setAba] = useState("historico"); // historico | nova
  const textareaRef = useRef(null);

  // Form nova comunicação
  ...
  useEffect(() => {
    if (clienteSelecionado) {
      carregarComunicacoes(clienteSelecionado.id);
    } else {
      carregarComunicacoes(null);
    }
  }, [clienteSelecionado]);
  ...
  async function carregarClientes() {
    ...
    const { data } = await supabase
      .from("clientes")
      .select("id, nome, telefone, email, regime_tributario, honorario_mensal")
      .order("nome");
    setClientes(data || []);
  ...
  function abrirNovaComm(clientePreSelecionado = null) {
    ...
    if (clientePreSelecionado) {
      setClienteParaForm(clientePreSelecionado);
    }
    setModalTipo("nova");
    setModalAberto(true);
  }
  ...
  function setClienteParaForm(cliente) {
  ...
  async function gerarMensagemComIA() {
    ...
    setGerandoIA(true);
    try {
      const prompt = TEMPLATES[form.template]?.promptIA(clienteForm, form.canal);
  ...
  async function abrirEmail() {
    if (!clienteForm?.email || !form.mensagem) return;
    const mailto = `mailto:${clienteForm.email}?subject=${encodeURIComponent(form.assunto || TEMPLATES[form.template]?.assuntoPadrao || "CARSANT Contabilidade")}&body=${encodeURIComponent(form.mensagem)}`;
  ...
  const commsFiltradas = comunicacoes.filter((c) => {
    ...
    <div className="flex h-full min-h-screen bg-gray-50">
      ...
      <div className="w-72 bg-white border-r border-gray-200 flex flex-col">
        ...
        <button
          ...
          className={`w-full text-left px-4 py-3 text-sm border-b border-gray-100 flex items-center gap-2 transition-colors ${
            !clienteSelecionado
          ...
          {loadingClientes ? (
            ...
            clientesFiltrados.map((c) => (
              ...
              >
                ...
                <div className="text-xs text-gray-400 mt-0.5">{c.regime_tributario}</div>
              </button>
            ))
          )}
        </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {clienteSelecionado && (
              <button
                onClick={() => abrirNovaComm(clienteSelecionado)}
          ...
          {loadingComms ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-400 text-sm mt-1">
                Clique em "+ Nova" para registrar a primeira
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {commsFiltradas.map((comm) => (
                <div
                  key={comm.id}
                  onClick={() => visualizarComm(comm)}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {comm.assunto && (
                          <p className="text-sm font-medium text-gray-700 mt-1">{comm.assunto}</p>
                        )}
                        <p className="text-sm text-gray-500 mt-1 line-clamp-2">{comm.mensagem}</p>
                        {comm.proximo_passo && (
                          <p className="text-xs text-blue-600 mt-1.5">
                            → Próximo passo: {comm.proximo_passo}
                          </p>
                        )}
                      </div>
                    </div>
                    <div className="flex flex-col items-end gap-2 shrink-0">
                      <span
                        className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_CORES[comm.status] || "bg-gray-100 text-gray-600"}`}
                      >
                        {comm.status}
                      </span>
                      ...
                      {comm.profiles?.nome && (
                        <span className="text-xs text-gray-400">{comm.profiles.nome}</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-gray-800">
                    {CANAL_ICONES[comunicacaoAtual.canal]} Comunicação
                  </h2>
                  <button
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium text-gray-800 mt-1">
                        {comunicacaoAtual.clientes?.nome}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium text-gray-800 mt-1 capitalize">
                        {CANAL_ICONES[comunicacaoAtual.canal]} {comunicacaoAtual.canal}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="mt-1">
                        <span
                          className={`text-xs px-2 py-1 rounded-full font-medium ${STATUS_CORES[comunicacaoAtual.status]}`}
                        >
                          {comunicacaoAtual.status}
                        </span>
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium text-gray-800 mt-1">
                        {new Date(comunicacaoAtual.created_at).toLocaleString("pt-BR")}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="mt-2 bg-gray-50 rounded-xl p-4 text-sm text-gray-700 whitespace-pre-wrap border border-gray-100">
                      {comunicacaoAtual.mensagem}
                    </div>
                  </div>
                ...
                <div className="flex gap-3 mt-6">
                  {comunicacaoAtual.canal === "whatsapp" &&
                    comunicacaoAtual.clientes?.telefone && (
                      <a
                        href={`https://wa.me/55${formatarTelefone(comunicacaoAtual.clientes.telefone)}?text=${encodeURIComponent(comunicacaoAtual.mensagem)}`}
                        target="_blank"
                        rel="noreferrer"
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {clienteForm && (
                      <div className="mt-1.5 flex gap-3 text-xs text-gray-500">
                        ...
                        {clienteForm.email && <span>✉️ {clienteForm.email}</span>}
                      </div>
                    )}
                  </div>
                  ...
                  {form.canal === "email" && (
                    <div>
                      ...
                      <input
                        ...
                        value={
                          form.assunto ||
                          (form.template ? TEMPLATES[form.template]?.assuntoPadrao || "" : "")
                        }
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {form.template && clienteForm && (form.canal === "whatsapp" || form.canal === "email") && (
                        <button
                          ...
                          disabled={gerandoIA}
                          className="text-xs bg-purple-100 text-purple-700 px-3 py-1 rounded-lg hover:bg-purple-200 font-medium disabled:opacity-50 flex items-center gap-1"
                        >
                          {gerandoIA ? (
                            <>
                    ...
                    <textarea
                      ...
                      placeholder={
                        form.canal === "telefone"
                          ? "Descreva o que foi tratado na ligação..."
                          : form.canal === "presencial"
                          ? "Descreva o que foi tratado no atendimento presencial..."
                ...
                <div className="flex gap-3 mt-6 pt-4 border-t border-gray-100">
                  ...
                  {form.canal === "email" && (
                    <button
                      onClick={abrirEmail}
                      disabled={!form.cliente_id || !form.mensagem || !clienteForm?.email}
                  ...
                  <button
                    onClick={() => setModalAberto(false)}
                    className="text-gray-400 px-4 py-2.5 rounded-xl text-sm hover:bg-gray-50"
                  >
                    Cancelar
                  </button>
                </div>
                ...
                {form.canal === "whatsapp" && !clienteForm?.telefone && form.cliente_id && (
                  <p className="text-xs text-red-500 mt-2">
                    ⚠️ Este cliente não tem telefone cadastrado. Adicione em Clientes antes de enviar.
                  </p>
                )}
                {form.canal === "email" && !clienteForm?.email && form.cliente_id && (
                  <p className="text-xs text-red-500 mt-2">
                    ⚠️ Este cliente não tem e-mail cadastrado. Adicione em Clientes antes de enviar.
                  </p>
                )}
              </div>
            )}
